...
| `docConstructor.js` | Pure model behind the Doc Viewer's **Constructor** view (`components/DocConstructor`): parses a generated document's line-based source into sections → pieces → `[[role.field]]` blanks, composes it back (`template` keeps blanks, `text` has them filled), fills a party from an identity record and settles gender / county-vs-sector / house-vs-flat clauses. A `.docx` opens in `DocxWorkspace` (DocViewer.jsx) with a Constructor ⇄ Word-preview switch; saving goes through the advisor's `saveConstructorVersion` as a new version. |
