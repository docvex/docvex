...
// Design handoff (docvex-activity-notification-redesign); the data now comes
// from the local activity log (lib/activityLog.js → lib/activityMetrics.js).
// Scope = signed-in user across all their projects ("my progress").
//
// Layout (all charts inline SVG / CSS — no chart lib):
//   Row 1 (hero) : This week at a glance — headline + verb tiles + 14-day trend
//   Row 2 (3col) : File-activity KPIs | 28-day heatmap | What I do most
...
function KpiStackCard({ m }) {
  const k = m.kpis;
  const filesDelta = pctDelta(k.filesTouched.value, k.filesTouched.prev);
  const projDelta = pctDelta(k.projectsActive.value, k.projectsActive.prev);
  const sub = k.filesTouched.sub;
  return (
    <div className="m-card">
      <div className="m-card-head">
      ...
      <div className="m-kpis">
        <div className="m-kpi">
          <span className="m-kpi-label">Files I touched</span>
          <span className="m-kpi-value">{k.filesTouched.value}</span>
          ...
          <Sparkline values={k.filesTouched.spark} />
        </div>
        <div className="m-kpi">
        ...
        <div className="m-kpi">
          <span className="m-kpi-label">AI assists</span>
          <span className="m-kpi-value">{k.aiAssists.value}</span>
          <span className="m-kpi-meta">
            {k.aiAssists.prev === 0
              ? <span className="m-delta is-flat">{k.aiAssists.value ? 'new' : 'none'}</span>
              : <Delta value={pctDelta(k.aiAssists.value, k.aiAssists.prev)} />}
          </span>
          <span className="m-kpi-sub">{k.aiAssists.detail}</span>
        </div>
      </div>
    </div>
  );
...
// ── What I do most (per-action breakdown) ─────────────────────────────
// Replaced the "People I worked with" card — its sources (cloud files +
// change requests) were dropped in migration 031 and could never populate.
function ActionsCard({ m }) {
  const rows = m.actionBreakdown || [];
  const max = Math.max(...rows.map((r) => r.count), 1);
  const total = rows.reduce((s, r) => s + r.count, 0);
  return (
    <div className="m-card">
      <div className="m-card-head">
        <h3 className="m-card-title"><span className="m-card-title-icon">{GlyphPeople}</span>What I do most</h3>
        <span className="m-card-period">{total} action{total === 1 ? '' : 's'} · 28 d</span>
      </div>
      {rows.length === 0 ? (
        <div className="m-empty">No file activity yet — create, import or edit a file and it shows up here.</div>
      ) : (
        <div className="m-files-body">
          {rows.slice(0, 6).map((r) => (
            <Tooltip key={r.action} content={`${r.label} — ${r.count} time${r.count === 1 ? '' : 's'} in the last 28 days`}>
              <div className="m-file-row">
                <div className="m-file-meta">
                  <span className="m-file-name">{r.label}</span>
                </div>
                <span className="m-file-edits">{r.count}</span>
                <div className="m-file-bar"><span style={{ width: `${(r.count / max) * 100}%` }} /></div>
              </div>
            </Tooltip>
          ))}
        </div>
...
export default function ActivityMetrics({ userId }) {
  ...
  const [tick, setTick] = useState(0); // bumped when the activity log changes

  useEffect(() => {
    if (!userId) return undefined;
    let cancelled = false;
    if (tick === 0) setState('loading'); // live refreshes swap in place, no flash
  ...
  // Live refresh — a new event (from this window or another, e.g. the
  // doc-viewer) recomputes the strip after a short debounce.
  useEffect(() => {
    if (!userId) return undefined;
    let handle = null;
    const unsub = subscribeActivity(userId, () => {
      clearTimeout(handle);
      handle = setTimeout(() => setTick((t) => t + 1), 400);
    });
    return () => { clearTimeout(handle); unsub?.(); };
  }, [userId]);
