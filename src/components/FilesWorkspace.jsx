...
const FolderGlyph = (
  <svg className="fx-folder-glyph" width="42" height="42" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
function extCategory(ext) {
  ...
  if (['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg', 'heic', 'bmp', 'tif', 'tiff', 'ai', 'psd'].includes(e)) return 'img';
  if (['mp4', 'mov', 'avi', 'mkv', 'webm', 'm4v'].includes(e)) return 'vid';
  if (['txt', 'md', 'rtf', 'log'].includes(e)) return 'txt';
  return 'gen';
}
const EXT_GLYPH_LABEL = { pdf: 'PDF', doc: 'DOC', xls: 'XLS', ppt: 'PPT', zip: 'ZIP', img: 'IMG', vid: 'MP4', txt: 'TXT', gen: 'FILE' };
    ...
    <>
      ...
      >
        ...
        <span className="fx-tile-thumb">
          {isFolder ? FolderGlyph : <FileThumbnail descriptor={item.descriptor} glyph={<ExtGlyph ext={item.ext} />} />}
        </span>
        <span>
      ...
      >
        <span className="fx-list-name">
          <span className="fx-list-thumb">
            {isFolder ? <Icon name="folder" size={15} /> : <FileThumbnail descriptor={item.descriptor} glyph={<ExtGlyph ext={item.ext} />} />}
          </span>
...
export default function FilesWorkspace({
  ...
  const hasUnpublished = branch?.state === 'changes' && draftCount > 0;
    ...
    <div className="fx-page">
      ...
      <div className="fx-window">
        ...
        >
          ...
          {tab === 'team' ? (
            // Team files is the read-only cloud/main view — its primary
            // action is to jump to your local drafts to edit.
            <button className="fx-tb-btn is-primary" onClick={() => onTabChange('drafts')}>
            ...
            <>
              ...
              {branch?.state === 'behind' && (
                <button className="fx-tb-btn is-primary" onClick={() => onGetUpdates?.()} disabled={!hasLocalFolder}>
        ...
        {/* Bottom action bar — file operations (moved down from the top)
            on the left, item count + branch state on the right. */}
